...
EdgeObserver.prototype = {
  ...
  external: function(cb) {
    this._bidi.from.external.on('in', wrap('from', cb));
    this._bidi.to.external.on('out', wrap('to', cb));
  },

  internal: function(cb) {
    this._bidi.from.internal.on('in', wrap('to', cb));
    this._bidi.to.internal.on('out', wrap('from', cb));
  }
